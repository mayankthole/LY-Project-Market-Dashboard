...
# Read Google Sheet ID from environment (no default)
...
def check_order_status(kite, order_id):
    ...
    try:
        ...
        if order_history and len(order_history) > 0:
            print(f"Found {len(order_history)} status entries for order {order_id}")
            ...
            print(f"All statuses for order {order_id}:")
            for i, status_item in enumerate(sorted_history):
                status = status_item.get('status', 'UNKNOWN')
                timestamp = status_item.get('exchange_timestamp', '')
                print(f"  {i+1}. {status} at {timestamp}")
            ...
            print(f"Using LAST status: {status} at {exchange_timestamp}")
            ...
            if action_status == "ORDER_PLACED" and order_id:
                try:
                    ...
                    if status:
                        ...
                        # Update status in column H (index 7) and timestamp in column I (index 8)
                        # Assuming columns: A=symbol, B=direction, C=quantity, D=action_status, E=timestamp, F=limit_price, G=order_id, H=order_status, I=status_timestamp
                        sheet.update(range_name=f"H{row_num}:I{row_num}", values=[[status, timestamp]])
                        print(f"Updated status for order {order_id}: {status} at {timestamp}")
                        updated_count += 1
                    else:
...
def process_place_orders_with_kite(kite, sheet_id):
    ...
    try:
        ...
        for idx in range(1, len(rows)):
            ...
            # On success, write action_status and timestamp
            if order_id:
                row_num = idx + 1  # 1-based row in sheet
                timestamp = get_indian_timestamp()
                try:
                    # Update action_status, timestamp, limit price, and order id in one call (D..G)
                    sheet.update(range_name=f"D{row_num}:G{row_num}", values=[["Order_Placed", timestamp, limit_price, order_id]])
                    placed_count += 1
                except Exception as e:
                    print(f"Failed updating action_status for row {row_num}: {e}")
...
def hello_http(request):
    try:
        ...
        # Read Google Sheet ID from environment
        SHEET_ID = os.getenv("SHEET_ID")
        if not SHEET_ID:
            return ("SHEET_ID environment variable is not set.", 500, {"Content-Type": "text/plain"})
